...
function SpeciesCard({ species }: any) {
  ...
  return (
    <div className="card mb-4 shadow-sm" style={{ position: "relative" }}>
      <Link
        to={`${match.url}/${species.name.replaceAll(" ", "-")}`}
        className="card-link"
      >
        <span
          style={{
...
function Specie() {
  ...
  let specie = SPECIES.find(
    (specie) => specie.name === specieId.replaceAll("-", " ")
  );
  if (specie) {
    return (
      <div className="bg-light">
        <main className="py-5 container" style={{ height: "100%" }}>
          <h1 className="text-center">{specie.name} </h1>
          <h2 className="text-center text-muted font-italic">
            {specie.genus + " " + specie.species}{" "}
          </h2>
          <img
            className="py-5"
            src={specie.imagePath}
            width="100%"
            alt={specie.name}
          ></img>
          <h3>Description:</h3>
          <p>{specie.physicalDescription.replace(/<(.|\n)*?>/g, "")}</p>
          <h3>Habitat:</h3>
          <p>{specie.habitatDescription.replace(/<(.|\n)*?>/g, "")}</p>
          <h3>Biology:</h3>
          <p>{specie.biology.replace(/<(.|\n)*?>/g, "")}</p>
          <h3>Harvest:</h3>
          <p>{specie.harvest.replace(/<(.|\n)*?>/g, "")}</p>
          <h3>Additional Information:</h3>
          <ul>
            <li>Region: {specie.region}</li>
            <li>NOAA Fishing Rate: {specie.fishingRate}</li>
            <li>IUCN Population Status: {specie.populationStatus}</li>
            <li>Fishing Impacts: {specie.fishingImpacts}</li>
          </ul>
        </main>
      </div>
    );
  ...
  return (
    <div className="py-5 container">
      <h3 className="text-center">Species not found.</h3>
    </div>
  );
}
