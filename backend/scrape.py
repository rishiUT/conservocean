...
def scrapeFish(limit, offset):
    limit_size = limit
    offset_size = offset
    fishes = requests.get("https://fishbase.ropensci.org/species?limit="+str(limit)+"&offset="+str(offset))
        ...
        if fish["FBname"] is not None and fish["image"] is not None:
            ...
            if iucn_code != "N.E.":
                fish_object = Fish(scientific_name=fish["Genus"]+" "+fish["Species"],
                        ...
                        habitat=str(fish["Fresh"])+" "+str(fish["Brack"])+" "+str(fish["Saltwater"]),
                ...
                while True:
                    try:
                        fish_location = requests.get("https://bison.usgs.gov/api/search.json?species="+fish["Genus"]+" "+fish["Species"]+"&type=scientific_name")
                        break
                    ...
                    try:
                        ...
                        if waterlist != []:
                            ...
                            for water in waterlist:
                                water_num += 1
                                fish_object.location.append(BodiesOfWater.query.get(water))
                    except KeyError as error:
                        pass
                if add:
    ...
    for bodyofwater in water_json:
        if bodyofwater["placeType"] in valid_water:
            ...
            if bodyofwater["minLatitude"] != None and bodyofwater["maxLatitude"] != None and bodyofwater["minLongitude"] != None and bodyofwater["maxLongitude"] != None and bodyofwater["preferredGazetteerName"] not in water_dict:
                calcSize = calc_dist(bodyofwater["minLatitude"], bodyofwater["maxLatitude"], bodyofwater["minLongitude"], bodyofwater["maxLongitude"])
                ...
                id_list.append(water_dict[bodyofwater["preferredGazetteerName"]])
    return id_list
