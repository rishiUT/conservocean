...
def filterFish(args, fish_list):
    ...
    Filters table of Fish based on passed parameters args.
        ...
        Parameters:
            args: (dict) Dictionary containing various parameters defined in our
                         API documentation.
    ...
    return {"total_fish_count": len(fish_list),
...
def filterHuman(args, human_list):
    ...
    for impact in human_list:
        ...
        if args['lat_min'] != None:
            if (impact.latitude < args['lat_min']) \
                or (impact.latitude > args['lat_max']):
                add = False

        if args['long_min'] != None:
            if (impact.longitude < args['long_min']) \
    ...
    return {"total_impact_count": len(human_list),
...
def filterWater(args, water_list):
    ...
    # Check for matching min and max pairs
    if checkArgs(args, 'lat_max', 'lat_min') or \
    ...
    for water in water_list:
        ...
        if args['lat_min'] != None:
            if water.latitude < args['lat_min'] \
               or water.latitude > args['lat_max']:
                add = False

        if args['long_min'] != None:
            if water.longitude < args['long_min'] \
               or water.longitude > args['long_max']:
                add = False
    ...
    return {"total_water_count": len(water_list),
